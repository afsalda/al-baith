    ...
    try {
        ...
        try {
            ...
            const adminEmailPromise = resend.emails.send({
                ...
                subject: `🏨 New Booking - ${name} | ${room_type}`,
                html: `
                    <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
                        <div style="background: linear-gradient(135deg, #1e3a5f 0%, #2563eb 100%); padding: 32px 24px; text-align: center;">
                            ...
                            <p style="color: #93c5fd; margin: 8px 0 0; font-size: 14px;">Admin Notification</p>
                        </div>
                        <div style="padding: 32px 24px;">
                            <table style="width: 100%; border-collapse: collapse;">
                                <tr><td style="padding: 10px 0; color: #6b7280; font-size: 14px; width: 140px;">Guest</td><td style="padding: 10px 0; color: #111827; font-size: 14px; font-weight: 600;">${name}</td></tr>
                                <tr style="background: #f9fafb;"><td style="padding: 10px 8px; color: #6b7280; font-size: 14px;">Email</td><td style="padding: 10px 8px; color: #111827; font-size: 14px;">${email}</td></tr>
                        ...
                        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 640px; margin: 0 auto; background: #fffdf7; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 40px rgba(0,0,0,0.08); border: 1px solid #f0e6cc;">
                            ...
                            <div style="padding: 28px 32px;">
                                ...
                                <div style="padding: 20px; background: linear-gradient(135deg, #1a1a2e, #0f3460); text-align: center;">
                                    <p style="margin: 0; color: #d4af37; font-size: 11px; text-transform: uppercase; letter-spacing: 2px;">Total Amount</p>
                                    <p style="margin: 6px 0 0; color: #FFD700; font-size: 32px; font-weight: 700; font-family: Georgia, 'Times New Roman', serif;">₹${totalPrice.toLocaleString()}</p>
                                    <p style="margin: 4px 0 0; color: #9ca3af; font-size: 12px;">inclusive of all taxes</p>
                                </div>
                            </div>
                        </div>
            ...
            const [adminResult, customerResult] = await Promise.allSettled([adminEmailPromise, customerEmailPromise]);

            if (adminResult.status === 'fulfilled') {
                const { data: aData, error: aErr } = adminResult.value;
                if (aErr) console.error('[API] Admin email error:', JSON.stringify(aErr));
                else console.log('[API] Admin notification sent:', JSON.stringify(aData));
            } else {
                console.error('[API] Admin email failed:', adminResult.reason);
            }

            if (customerResult.status === 'fulfilled') {
                const { data: cData, error: cErr } = customerResult.value;
                if (cErr) console.error('[API] Customer email error:', JSON.stringify(cErr));
                else console.log('[API] Customer confirmation sent to', email, ':', JSON.stringify(cData));
            } else {
                console.error('[API] Customer email failed:', customerResult.reason);
            }
